...
/* Used for HW validation: allow all SFR in GET/SET/WFX */
int allow_all_sfr = 0;
...
/* The target core (0: KV3-1, 1: KV3-2) */
int kvx_core = -1;
...
bool
kvx_parse_name (const char *name, struct expressionS *e)
{
  struct symbol *sym;
...
static int
tokenize_arguments (char *str, expressionS tok[], char *tok_begins[],
		    int ntok)
{
...
#define MATCH_KVX_REGFILE(tok,is_regfile) \
  ...
  for (jj = 0; jj < ntok; jj++)
	...
	MATCH_KVX_REGFILE (tok[jj], IS_KVX_REGFILE_GPR) case RegClass_kv3_pairedReg:
	MATCH_KVX_REGFILE (tok[jj], IS_KVX_REGFILE_PGR) case RegClass_kv3_quadReg:
	MATCH_KVX_REGFILE (tok[jj], IS_KVX_REGFILE_QGR) case RegClass_kv3_systemReg:
	...
	MATCH_KVX_REGFILE (tok[jj], IS_KVX_REGFILE_SFR) case RegClass_kv3_coproReg:
	case RegClass_kv3_coproReg0M4:
	case RegClass_kv3_coproReg1M4:
	case RegClass_kv3_coproReg2M4:
	case RegClass_kv3_coproReg3M4:
	MATCH_KVX_REGFILE (tok[jj], IS_KVX_REGFILE_XCR) case RegClass_kv3_blockReg:
	...
	MATCH_KVX_REGFILE (tok[jj], IS_KVX_REGFILE_X8R) case RegClass_kv3_buffer16Reg:
	MATCH_KVX_REGFILE (tok[jj], IS_KVX_REGFILE_X16R) case RegClass_kv3_buffer32Reg:
	MATCH_KVX_REGFILE (tok[jj], IS_KVX_REGFILE_X32R) case RegClass_kv3_buffer64Reg:
	MATCH_KVX_REGFILE (tok[jj], IS_KVX_REGFILE_X64R) case Immediate_kv3_pcrel17:
	...
	case Immediate_kv3_unsigned6:
	  ...
	  if (tok[jj].X_op == O_constant)
	      ...
	      if (opdef->flags & kvxWRAPPED)
		{
		  ...
		  int shift =
		    (sizeof (signed long long) * 8) - opdef->width - 1;
		  ...
		  if ((signed_value & high_mask) == 0)
		    {
		      signed_value =
			(signed_value << (64 - opdef->width)) >> (64 -
								  opdef->
								  width);
		    }
		}
	      ...
	      if (opdef->bias != 0)
		{
	      ...
	      match_signed = (((signed_value >> opdef->shift) >= min)
			      && ((signed_value >> opdef->shift) <= max));
	      match_unsigned =
		(((unsigned_value >> opdef->shift) & mask) ==
		 (unsigned_value >> opdef->shift));
		  ...
		  ((!(opdef->flags & kvxSIGNED) && match_unsigned) ||	/* Match unsigned and operand is unsigned or wrapped */
		   (((opdef->flags & kvxSIGNED)
		     || (opdef->flags & kvxWRAPPED)) && match_signed)))
		{		/* Match signed and operand is signed or wrapped */
		  return MATCH_NOT_FOUND;
		}
	      break;
	    }
 ...
 */
static int
insert_operand (kvxinsn_t * insn, kvxbfield * opdef, const expressionS * arg)
{
  ...
  switch ((int) arg->X_op)
      ...
      if (insn->nfixups == 0)
	{
	  ...
	  struct pseudo_func_s *pf =
	    kvx_get_pseudo_func2 (arg->X_op_symbol, opdef);
...
int
is_constant_expression (expressionS * exp)
{
  int retval = false;
 ...
 */
static Bundling
find_bundling (const kvxinsn_t * insn)
{
...
static int
find_reservation (const kvxinsn_t * insn)
{
	...
	if (check_resource_usage)
	  {
	    const int reservation_table_len =
	      (kv3_reservation_table_lines * kv3_resource_max);
		...
		const int *reservation_table =
		  kv3_reservation_table_table[reservation];
		...
		for (j = 0; j < kv3_resource_max; j++)
		  if (resources_used[(i * kv3_resource_max) + j] >
		      resources[j])
		    {
		      int v = resources_used[(i * kv3_resource_max) + j];
		      free (resources_used);
		      as_fatal
			("Resource %s over-used in bundle: %d used, %d available",
...
kvx_make_nops (char *buf, bfd_vma bytes)
{
  ...
  if (!nop_single)
    {
      const kv3opc_t *opcode =
	(kv3opc_t *) str_hash_find (kvx_opcode_hash, "nop");
